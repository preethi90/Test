...
Func _AbbrevManager()
	...
	#region ; Paths
		...
		EndIf
		Local $sUser_KeyWordPath = $SciTEUserDir & "\au3.keywords.user.abbreviations.properties"
		Local $sSciTE_AbbrevPath = $SciTEUserDir & "\au3abbrev.properties"
		Local $sUser_AbbrevPath = $SciTEUserDir & "\au3userabbrev.properties"

		Local $sUser_BackUpPath
		...
		If StringInStr(@ScriptDir, @ProgramFilesDir) Then
			...
			If Not FileExists($SciTEUserDir & "\SciTEConfig\AbbrevMan") Then DirCreate($SciTEUserDir & "\SciTEConfig\AbbrevMan")
			$sUser_BackUpPath = $SciTEUserDir & "\SciTEConfig\AbbrevMan\Abbrev.txt"
		Else
			; Can use working directory
			$sUser_BackUpPath = @ScriptDir & "\Abbrev.txt"
		EndIf
...
#region ; Abbreviation Functions
	...
	Func _Add_NewAbbrev($aGUI_Control, ByRef $aUser_Abbrev, ByRef $aSciTE_Abbrev, $sAbbrev_Name, $sAbbrev_Text, $fUser, ByRef $fChange)
		...
		; Insert abbrev
		_ArrayAdd($aUser_Abbrev, $sAbbrev_Name & "=" & $sAbbr)
		$aUser_Abbrev[0] += 1
		_ArraySort($aUser_Abbrev, 0, 1)
...
#region ; Internal Functions
	...
	Func _SearchArray($aArray, $sAbbrev_Name)
		...
		For $i = 1 To $aArray[0]
			If StringRegExp($aArray[$i], "^" & $sAbbrev_Name & "=.*$") Then
				Return $i
			EndIf
		Next
		Return -1
