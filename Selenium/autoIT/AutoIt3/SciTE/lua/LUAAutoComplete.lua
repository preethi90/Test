--------------------------------------------------------------------------------
-- This library provides some convenience when working in LUA scripts.  It
-- causes the AutoComplete and CallTips() to behave more intelligently.
--------------------------------------------------------------------------------
LUAAutoComplete = EventClass:new(Common)
...
--------------------------------------------------------------------------------
function LUAAutoComplete:OnStartup()
...
--
--------------------------------------------------------------------------------
function LUAAutoComplete:OnUpdateUI()
	...
	if self:IsLexer(SCLEX_LUA) and self.Check_Calltip then
		self.Check_Calltip = false
		local style = editor.StyleAt[editor.CurrentPos-1]
		-- Only update calltip when an operator is typed
		if style ~= SCE_LUA_OPERATOR then return false end
...
--------------------------------------------------------------------------------
function LUAAutoComplete:OnChar(c)
	-- Make sure we only do this for the LUA lexer.
	if self:IsLexer(SCLEX_LUA) then
		if props['autocomplete.LUA.disable'] == "1" then
		...
		if ((c == "," or c == "(")) then
			-- set variable to 1 and let LUAAutoComplete:OnUpdateUI handle the rest as only then the StyleAt is proper set
...
--------------------------------------------------------------------------------
function LUAAutoComplete:IsValidStyle(style)
...
--------------------------------------------------------------------------------
function LUAAutoComplete:IsValidFuncChar(c)
...
--------------------------------------------------------------------------------
function LUAAutoComplete:IsQuoteChar(c)
