#NoTrayIcon
Global Const $VERSION = "19.102.1901.0"
...
#AutoIt3Wrapper_Res_Fileversion=19.102.1901.0
...
ElseIf Not StringInStr($CMDLINERAW, "/watcher") _
		...
		Then
	AutoItWinSetTitle("AutoIt3Wrapper_Main")
...
;-------------------------------------------------------------------------------------------
While Not FileExists($ScriptFile_In) And $Option <> "RunAdmin" ; Or StringRight($ScriptFile_In, 4) <> '.au3'
...
Func Run_the_Script($Init = 0)
	...
	If $Option <> "RunAdmin" Then
		__ConsoleWrite("+>Setting Hotkeys...")
...
Func ShowStdOutErr($l_Handle, $ShowConsole = 1, $Replace = "", $ReplaceWith = "")
	...
	Do
		...
		$tot_out &= $Line2
		If $Line2 <> "" And $UseMailSlot Then
			_MailSlotWrite($sMailSlotConsole, $Line2)      ;Used when shelled script uses #requireAdmin
		EndIf
		If $ShowConsole Then
			ConsoleWrite($Line2)
		EndIf
...
Func RestartReqAdmin()
	...
	__ConsoleWrite("->" & @HOUR & ":" & @MIN & ":" & @SEC & " Script requires Admin rights while SciTE is at normal level -> Staring new AutoIt3Wrapper with #RequireAdmin to run the script." & @CRLF)
